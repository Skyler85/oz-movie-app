...
const SearchMovie = () => {
  ...
  const fetchSearchMovie = async (searchTerm) => {
    try {
      const response = await axios.get(
        `/search/movie?query=${searchTerm}&include_adult=false`
      );
  ...
  return (
    <>
      <h1 style={{ margin: '30px auto', textAlign: 'center' }}>'{debounceSearchTerm}'의 검색 결과</h1>
        ...
        <ul className='search-box'>
          {searchResult.length > 0 ? (
            searchResult.map((movie) => (
              <li
                key={movie.id}
                onClick={() => navigate(`/search/detail/${movie.id}`)}
              >
